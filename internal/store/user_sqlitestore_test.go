...
func TestCreateUser(t *testing.T) {
	t.Run("success - user is stored", func(t *testing.T) {
		...
		// act
		u, err := userStore.CreateUser(context.Background(), types.Admin, "testadmin", string(hash))
		...
		assert.Equal(t, "testadmin", u.Username)
		assert.Equal(t, string(hash), u.PasswordHash)
...
func TestUserSQLiteStore_CreateAuthSession(t *testing.T) {
	t.Run("success - auth session created", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.NotNil(t, as)
		assert.Equal(t, expectedAuthSession.AuthSessionID, as.AuthSessionID)
		assert.Equal(t, expectedAuthSession.AuthSessionUserID, as.AuthSessionUserID)
		assert.Equal(t, expectedAuthSession.AuthSessionExpires, as.AuthSessionExpires)
	})
	t.Run("failure - user id not found", func(t *testing.T) {
		// arrange
		var userID int64 = 432424
